...
def test_metadata_update_and_retrieval(storage):
    ...
    file_id = "test123"
    test_metadata = {"test_key": "test_value", "nested": {"key": "value"}}
    ...
    # Test nested update
    update_metadata = {"nested": {"new_key": "new_value"}}
    storage.update_metadata(file_id, update_metadata)

    expected_metadata = {
        "test_key": "test_value",
        "nested": {"key": "value", "new_key": "new_value"},
    }
...
def test_audio_duration_limit(storage):
    ...
    assert (
        len(saved_audio) <= storage.max_audio_secs * 1000
...
def test_audio_duration_storage(storage):
    ...
    assert metadata is not None
    assert (
        metadata["original_duration"] == original_duration_ms / 1000
    )  # Convert to seconds
    assert (
        metadata["duration"] == storage.max_audio_secs
    )  # Should be truncated to limit
    assert (
        metadata["duration"] < metadata["original_duration"]
