        ...
        if default_input.is_file():
            ...
            if extensions is None and file_pattern == "*":
                ...
                input_files = find_audio_files(
                    default_input, extensions=AUDIO_EXTENSIONS
                )
            else:
                # Use specified pattern and extensions
                if extensions:
                    for ext in extensions:
                        input_files.extend(
                            list(default_input.rglob(f"{file_pattern}{ext}"))
                        )
                else:
...
def find_files_by_pattern(
    directory: Union[str, pathlib.Path], pattern: str
) -> List[pathlib.Path]:
    """
...
def resolve_input_path(
    filename: str, input_dir: Optional[Union[str, pathlib.Path]] = None
) -> pathlib.Path:
    """
...
def find_beats_file_for_audio(
    ...
    if beats_file is None:
        for match in pathlib.Path(output_base_dir).rglob(
            f"{audio_path.stem}_beats.txt"
        ):
            beats_file = match
            break
...
def batch_process(
    files: List[pathlib.Path], process_func: Callable, verbose: bool = True, **kwargs
) -> List:
    """
