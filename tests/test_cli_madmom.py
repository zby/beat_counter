...
CLI_TEST_OUTPUT_DIR = TEST_DATA_DIR / "cli_tests_madmom"  # Separate directory for madmom tests
...
if not SAMPLE_AUDIO_PATH.is_file():
    pytest.fail(
        ...
        f"Please ensure the file exists or adjust SAMPLE_AUDIO_PATH in test_cli_madmom.py",
        pytrace=False,
    )
...
def cli_test_setup() -> pathlib.Path:
    """Sets up the tests/data/cli_tests_madmom directory for CLI testing.
...
def test_detect_beats_default_output(cli_test_setup):
    ...
    # Use min_measures=1 for testing with short audio samples
    result = run_cli(["detect-beats", str(audio_file), "--algorithm", "madmom", "--min-measures", "1"]) # No -o specified
...
def test_detect_beats_explicit_output(cli_test_setup):
    ...
    output_beats_file = cli_test_setup.parent / "explicit_output_madmom.beats"
...
def test_detect_beats_audio_not_found():
    ...
    result = run_cli(["detect-beats", str(non_existent_file), "--algorithm", "madmom"])
    assert result.returncode != 0
...
def test_detect_beats_batch_dir_not_found():
    ...
    result = run_cli(["detect-beats-batch", str(non_existent_dir), "--algorithm", "madmom"])
    assert result.returncode != 0
...
def test_detect_beats_batch_empty_dir(cli_test_setup):
    ...
    empty_dir = cli_test_setup.parent / "empty_dir_madmom"
    if empty_dir.exists():
        shutil.rmtree(empty_dir)
    empty_dir.mkdir()

    result = run_cli(["detect-beats-batch", str(empty_dir), "--algorithm", "madmom"])
    assert result.returncode != 0
...
def test_generate_video_explicit_output(cli_test_setup, beats_file_for_video):
    ...
    output_video_file = cli_test_setup.parent / "explicit_output_madmom.mp4"
...
def test_generate_video_batch_default_output(cli_test_setup):
    ...
    if not beats_file.is_file():
        # Use min_measures=1 for testing with short audio samples
        result_beats = run_cli(["detect-beats", str(audio_file_path), "--algorithm", "madmom", "--min-measures", "1"])
...
def test_generate_video_batch_explicit_output(cli_test_setup):
    ...
    audio_file_path = input_dir / SAMPLE_AUDIO_NAME
    beats_file = audio_file_path.with_suffix(".beats")
    output_dir = input_dir / "video_batch_output_madmom"
