...
class FileMetadataStorage:
    ...
    def __init__(self, config: StorageConfig):
        ...
        logger.info(
            f"File metadata storage initialized with base directory: {self.base_upload_dir}"
        )
    ...
    def update_metadata(self, file_id: str, metadata: Dict[str, Any]) -> None:
        ...
        if not lock_file.exists():
            try:
                ...
                logger.warning(
                    f"Failed to create lock file, proceeding without locking: {e}"
                )
            ...
            with open(lock_file, "r+") as lock_f:
                ...
                while True:
                    try:
                        ...
                        if elapsed > max_wait:
                            logger.warning(
                                f"Timeout waiting for lock, proceeding without lock: {file_id}"
                            )
                            break
                        time.sleep(0.1)
                    ...
                    if metadata_file.exists():
                        with open(metadata_file, "r") as f:
                            try:
                                ...
                                logger.warning(
                                    f"Invalid JSON in metadata file, starting with empty metadata: {file_id}"
                                )
    ...
    def check_ready_for_confirmation(self, file_id: str) -> bool:
        ...
        try:
            ...
            if not metadata:
                logger.info(
                    f"No metadata found for {file_id} during confirmation check."
                )
                return False
            ...
            if metadata.get("beat_detection_status") != "success":
                logger.info(
                    f"Beat detection status in metadata is not 'success' for {file_id}: {metadata.get('beat_detection_status')}"
                )
                return False
            ...
            if not pathlib.Path(beats_file_path_str).exists():
                logger.warning(
                    f"Beats file path found in metadata ({beats_file_path_str}), but file does not exist on disk for {file_id}"
                )
            ...
            logger.error(
                f"Error checking if file {file_id} is ready for confirmation: {e}"
            )
            return False
    ...
    def get_audio_file_path(
        self, file_id: str, file_extension: str = None
    ) -> pathlib.Path:
        ...
        if metadata and "file_extension" in metadata:
            return (
                self.get_job_directory(file_id) / f"audio{metadata['file_extension']}"
            )
    ...
    def save_audio_file(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to save in original format {file_extension}, falling back to MP3: {e}"
                )
            ...
            metadata = {
                ...
                "duration": len(truncated_audio)
                / 1000,  # Store truncated duration in seconds
